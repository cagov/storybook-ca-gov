export default function getScreenResize() {
  // Putting this in a special scope for just this component.
  window.componentDisplay = {};
  ...
  const getWindowSize = () => {
    ...
    if (window.componentDisplay !== undefined) {
      if (windowSize.width <= 576) {
        window.componentDisplay.displayType = "mobile";
        if (highDensity === true || isRetina === true) {
          window.componentDisplay.displayType = "retina";
        }
      } else if (windowSize.width > 576 && windowSize.width <= 768) {
        window.componentDisplay.displayType = "tablet";
      } else {
        window.componentDisplay.displayType = "desktop";
      }
    }
  };
